...
select count( * ) 
	from users u
	where country like 'Iceland' and last_connection like '112.%';
...
select first_name, last_name, country, last_connection from users u 
	where country like 'Iceland'
		and ( 
...
select first_name, last_name, country, last_connection from users u 
	where country like 'Iceland'
		and ( u.last_connection like '112.%' or u.last_connection like '28.%' or u.last_connection like '188.%')
...
select count(*) as total, country from users u 
	where country in ('Mexico', 'Honduras', 'Costa Rica')
	group by country 
